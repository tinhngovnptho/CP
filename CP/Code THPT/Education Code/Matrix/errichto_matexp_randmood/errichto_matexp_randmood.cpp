...
struct Matrix {
    ...
    Matrix(int _r = 0, int _c = 0) {
        r = _r;
        c = _c;
        mat.assign(r, vector<long double>(c, 0));
    }
    Matrix(vector<vector<long double> > _mat) {
        r = _mat.size();
        c = _mat[0].size();
        mat = _mat;
    }
    ...
    Matrix operator *(const Matrix &ot) const {
        ...
        FOR(i, 0, r - 1) FOR(j, 0, c - 1) {
            FOR(k, 0, c - 1) res.mat[i][j] += mat[i][k] * ot.mat[k][j];
        }
        return res;
    }
...
void process(void) {
    ...
    base = base.pow(n);
    cout << fixed << setprecision(10) << base.mat[0][0];
}

int main(void) {
	ios_base::sync_with_stdio(false); cin.tie(nullptr);

    file("errichto_matexp_randmood");
